          ...
          sales.map((sale) => (
            <div key={sale.id} className="bg-background-beige p-4 rounded-lg flex items-center justify-between shadow-sm">
              ...
              <div className="flex items-center space-x-2">
                <button onClick={() => onEdit(sale)} className="p-2 text-text-light hover:text-text-dark" aria-label={`Edit sale`}>
                  <EditIcon />
                </button>
                <button onClick={() => onDelete(sale.id)} className="p-2 text-red-400 hover:text-red-600" aria-label={`Delete sale`}>
                  <TrashIcon />
                </button>
              </div>
            </div>
